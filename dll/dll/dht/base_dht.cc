...
int BaseDht::Drop(NodePtr& node) {
    ...
    std::lock_guard<std::mutex> guard2(dht_mutex_);
    {
        ...
        if (iter != dht_.end()) {
            if ((*iter)->id != node->id) {
                DHT_ERROR("invalid node id[%s][%s][%llu][%llu]",
...

    {
        ...
        if (iter != node_map_.end()) {
            if (iter->second->id != node->id) {
                DHT_ERROR("invalid node id[%s][%s][%llu][%llu]",
                        common::Encode::HexEncode(iter->second->id).c_str(),
                        common::Encode::HexEncode(node->id).c_str(),
                        iter->second->dht_key_hash,
                        node->dht_key_hash);
                assert(false);
            }
...
int BaseDht::Bootstrap(const std::vector<NodePtr>& boot_nodes) {
    ...
    for (uint32_t i = 0; i < boot_nodes.size(); ++i) {
        ...
        DHT_ERROR("join[%s][%d][%s][%s] sent request.",
    ...
    join_res_con_.wait_for(lock, std::chrono::seconds(10), [this]() -> bool { return joined_; });
    if (!joined_) {
        DHT_ERROR("join error.");
        return kDhtError;
    }
    return kDhtSuccess;
}
...
void BaseDht::ProcessHeartbeatResponse(
        ...
        protobuf::DhtMessage& dht_msg) {
    LEGO_NETWORK_DEBUG_FOR_PROTOMESSAGE("end", header);
    DHT_ERROR("receive heartbeat response.");
    if (!CheckDestination(header.des_dht_key(), false)) {
...
void BaseDht::Heartbeat() {
    ...
    for (auto iter = tmp_dht_ptr->begin(); iter != tmp_dht_ptr->end(); ++iter) {
        ...
        transport::MultiThreadHandler::Instance()->transport()->Send(
                node->public_ip, node->public_port, 0, msg);
        DHT_ERROR("sent heartbeat request[%s][%d].", node->public_ip.c_str(), node->public_port);
