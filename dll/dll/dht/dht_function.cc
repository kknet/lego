...
uint32_t DhtFunction::PartialSort(const std::string& target, uint32_t count, Dht& dht) {
    uint32_t min_count = count;
    if (min_count > static_cast<uint32_t>(dht.size())) {
        min_count = static_cast<uint32_t>(dht.size());
    }
...
bool DhtFunction::Displacement(
    ...
    for (uint32_t index = dht.size() - 1; index >= 0; --index) {
...
NodePtr DhtFunction::GetClosestNode(
    ...
    auto closest_nodes(GetClosestNodes(dht, target, kDhtMaxNeighbors));
