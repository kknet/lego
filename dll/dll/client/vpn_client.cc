...
std::string VpnClient::Init(
    ...
    if (ConfigExists(conf_path)) {
        if (!config_.Init(conf_path)) {
            CLIENT_ERROR("init config_ failed!");
            config_.Set("lego", "prikey", std::string(""));
        }

        std::string priky("");
        if (!config_.Get("lego", "prikey", priky) || priky.empty()) {
            CLIENT_ERROR("config_[%s] invalid!", conf_path.c_str());
        } else {
            private_key = common::Encode::HexDecode(priky);
        }
    }

    config_.Get("lego", "first_instasll", first_install_);
    config_.Set("lego", "local_ip", local_ip);
    config_.Set("lego", "local_port", local_port);
    config_.Set("lego", "country", std::string("CN"));
    config_.Set("lego", "first_node", false);
    config_.Set("lego", "client", true);
    config_.Set("lego", "bootstrap", bootstrap);
    config_.Set("lego", "id", std::string("test_id"));
    std::string boot_net;
    config_.Get("lego", "bootstrap_net", boot_net);
    boot_net += "," + bootstrap;
    if (common::GlobalInfo::Instance()->Init(config_) != common::kCommonSuccess) {
    ...
    config_.Set("lego", "prikey", common::Encode::HexEncode(
            security::Schnorr::Instance()->str_prikey()));
    config_.Set("lego", "pubkey", common::Encode::HexEncode(
    ...
    config_.Set("lego", "id", common::Encode::HexEncode(
            common::GlobalInfo::Instance()->id()));
    std::string vpn_us_nodes;
    config_.Get("vpn", "US", vpn_us_nodes);
...
bool VpnClient::SetFirstInstall() {
    first_install_ = true;
    config_.Set("lego", "first_instasll", first_install_);
    config_.DumpConfig(config_path_);
    return true;
}
...
int VpnClient::InitNetworkSingleton() {
    if (network::Bootstrap::Instance()->Init(config_) != network::kNetworkSuccess) {
...
void VpnClient::DumpVpnNodes() {
    ...
    for (auto iter = vpn_nodes_map_.begin(); iter != vpn_nodes_map_.end(); ++iter) {
        ...
        config_.Set("vpn", iter->first, conf_str);
        country_list += iter->first + ",";
    }
    config_.Set("vpn", "country", country_list);
}
...
void VpnClient::DumpRouteNodes() {
    ...
    for (auto iter = route_nodes_map_.begin(); iter != route_nodes_map_.end(); ++iter) {
        ...
        config_.Set("route", iter->first, conf_str);
        country_list += iter->first + ",";
    }
    config_.Set("route", "country", country_list);
}

void VpnClient::ReadRouteNodesFromConf() {
    std::string country_list;
    config_.Get("route", "country", country_list);
    ...
    for (uint32_t i = 0; i < country_split.Count(); ++i) {
        ...
        config_.Get("route", country_split[i], route_nodes);
...
void VpnClient::ReadVpnNodesFromConf() {
    std::string country_list;
    config_.Get("vpn", "country", country_list);
    if (country_list.empty()) {
        return;
    }
    ...
    for (uint32_t i = 0; i < country_split.Count(); ++i) {
        ...
        config_.Get("vpn", country_split[i], vpn_nodes);
        if (vpn_nodes.empty()) {
            continue;
        }
...
void VpnClient::DumpBootstrapNodes() {
    ...
    if (!bootstrap_set.empty()) {
        ...
        config_.Set("lego", "bootstrap_net", boot_str);
        config_.DumpConfig(config_path_);
    }
